...
//
//  PRICING PRINCIPLE - a REALISTIC floor. Never a range, and never a best case.
//  A range anchors the customer to its low end and the shop has to argue upward
//  at the counter, so the output is a starting price plus a named list of what
//  can push it up.
//  But a floor built by assuming the cheapest answer to every unknown is just as
//  damaging, in the same direction: the customer hears a number that will never
//  happen, then meets a 40-60 percent higher one at the counter. So an unknown
//  resolves to the TYPICAL case - middle part tier, dual-mass flywheel on a
//  modern diesel, AGM battery on a 2015-or-newer car - and the quote SAYS which
//  assumption it made. See assumeKtl, assumeAgm and the collected `assumed` list.
//
...
// WHAT AN "I DON'T KNOW" COSTS.
//
// This used to resolve to the cheapest option every time, on the theory that a
// floor can only ever go up. That theory was half right and the implementation
// was wrong: a quote built from the cheapest answer to five different unknowns
// is not a floor, it is a best case that will essentially never happen. The
// customer then walks into the workshop and hears a number 40-60 percent
// higher, which is the exact "de a kalkulátor mást mondott" scene the floor was
// supposed to prevent.
//
// So an unknown now resolves to the TYPICAL case, not the cheapest one, and the
// quote says which assumption it made. The number is still labelled "-tól",
// because genuine extras (seized bolts, a worn disc) still exist - but it is a
// realistic starting price rather than a fantasy one.
...
// The two "I don't know" answers that move the price most, resolved to what is
// actually typical for the car in front of us rather than to whichever branch
// happens to be cheaper.
//
// A dual-mass flywheel on a modern diesel is the norm, not the exception, and
// it is a 180 000 Ft part. Assuming it away because the customer could not
// answer is how a clutch quote ends up half of what the invoice says.
function assumeKtl(sel) {
    if (sel.kuplung_ktl === "igen") return true;
    if (sel.kuplung_ktl === "nem") return false;
    return fuelOf(sel.engine) === "dizel" && yearOf(sel) >= 2005;
}
// Same logic for start-stop: on a 2015-or-newer car it is close to standard,
// and an AGM battery is nearly double a conventional one.
function assumeAgm(sel) {
    if (sel.akku_startstop === "igen") return true;
    if (sel.akku_startstop === "nem") return false;
    return yearOf(sel) >= 2015;
}
...
export const FIELDS = {
    ...
    year: {
        ...
        q: "Évjárat",
        hint: (sel) => {
            // The year drives different things for different jobs, so it says
            // the one that applies. A klímagáz explanation on an oil change
            // reads as a bot asking irrelevant questions - which is exactly how
            // it was read.
            if (hasJob(sel, "klima")) return "Ebből jön ki, melyik klímagáz megy bele - a 2017 utániakba a drágább R1234yf.";
            if (hasJob(sel, "fek")) return "Ebből derül ki, lehet-e benne elektromos kézifék.";
            if (hasJob(sel, "olajcsere")) return "Az évjárat és a motor együtt adja ki, mennyi olaj megy bele.";
            return "Elég nagyjából - a forgalmiban az első oldalon van.";
        },
        options: [
    ...
    olaj_szuro: {
        short: "Szűrők",
        q: "Melyik szűrők menjenek bele?",
        hint: "Az olajszűrő mindig cserélődik. Sok szervizben a többi szűrő is alapból benne van az olajcserében.",
        options: [opt("Minden szűrőt (levegő, pollen, üzemanyag)", "mind"), opt("Csak az olajszűrőt", "csak_olaj"), NT()],
    },
    fek_hol: {
        short: "Melyik fék",
        q: "Melyik tengelyen?",
    ...
    parts_tier: {
        ...
        hint: "Ez mozgatja a legtöbbet az áron. Ha nem tudod, a középső, márkás kategóriával számolok - azzal, amit a szerviz alapból beépítene.",
        options: [
            opt("Utángyártott - a legolcsóbb", "utangyartott"),
            opt("Márkás utángyártott (Bosch, TRW, Febi)", "markas"),
            opt("Gyári (OEM)", "gyari"),
            opt("Nem tudom, döntsék el önök", "nem_tudom"),
        ],
...
export function buildQuote(sel, { money, itemMoney = money }) {
    ...
    const ex = [];
    const flags = [];
    // Every place the customer said "nem tudom" and the price model had to pick
    // something. These are shown to the customer, because an assumption they
    // cannot see is exactly the thing that blows up at the counter.
    const assumed = [];
    const t = tier(sel);
    const cls = klass(sel);
    ...
    for (const j of priced) {
        ...
        if (j === "olajcsere") {
            ...
            const allFilters = sel.olaj_szuro !== "csak_olaj";
            if (allFilters) {
                addPart("Levegő-, pollen- és üzemanyagszűrő", "szuro_keszlet");
                if (sel.olaj_szuro !== "mind") assumed.push("**minden szűrőt** cserélünk - sok szervizben ez alapból az olajcsere része. Csak olajszűrővel olcsóbb.");
            }
            ...
            if (!allFilters) ex.push(`Levegő-, pollen- és üzemanyagszűrő, ha mégis kell: ${money(A.szuro_keszlet[t] * partsK(sel))}-tól.`);
        } else if (j === "fek") {
            ...
            const ktl = assumeKtl(sel);
            if (ktl) {
                addPart("Kettőstömegű lendkerék", "ktl_lendkerek");
                if (sel.kuplung_ktl !== "igen") {
                    assumed.push("**kettőstömegű lendkerék van benne** - ez egy 2005 utáni dízelnél a szokásos. Ha nincs, ennyivel olcsóbb lesz.");
                }
            } else {
                ex.push(`Ha a kettőstömegű lendkerék mégis kopott, cseréje ${money(A.ktl_lendkerek[t] * partsK(sel))}-tól. Ez a szétszerelés után derül ki.`);
            }
            ...
            const agm = assumeAgm(sel);
            if (agm && sel.akku_startstop !== "igen") {
                assumed.push("**start-stop rendszer van benne**, tehát AGM akkumulátor kell - egy 2015 utáni autónál ez a szokásos.");
            }
            ...
            // An unknown wheel size takes the middle band: most cars on the road
            // are on 17-18 inch rims, and guessing 16 makes the job look cheaper
            // than it will be.
            const size = ["m16", "m18", "m19"].includes(sel.gumi_meret) ? sel.gumi_meret : "m18";
            if (size !== sel.gumi_meret) assumed.push("**17-18 colos kerékkel** számoltam, mert ez a leggyakoribb.");
            const sizeK = size === "m19" ? 1.5 : size === "m18" ? 1.25 : 1;
            labourTotal += addLabour(
                `Gumiszerelés, centírozás (${sel.gumi_db === "2" ? "2" : "4"} kerék${sizeK > 1 ? ", 17-18 coll" : ""})`,
    ...
    if (sel.parts_tier === "nem_tudom" || !sel.parts_tier) {
        assumed.push("**márkás utángyártott alkatrésszel** (Bosch, TRW, Febi) számoltam - ezt szokta a szerviz alapból beépíteni. Olcsóbb utángyártottal kevesebb, gyárival több.");
    }
    if (sel.engine === "nem_tudom" || !sel.engine) {
        assumed.push("**benzines motorral** számoltam, mert nem tudtuk, melyik van benne. Dízelnél a munkadíj és az olajmennyiség is több.");
    }
    if (!yearOf(sel)) {
    ...
    return {
        ...
        includes: priced.length
            ? "a munkadíjat, a felsorolt alkatrészeket és az apróanyagot tartalmazza, a választott alkatrész-kategória árával számolva"
...
export const TEST_SCENARIOS = [
    {
        ...
        answers: {
            ...
            olaj_szuro: "Csak az olajszűrőt", parts_tier: "Márkás utángyártott (Bosch, TRW, Febi)",
        },
        min: 14000, max: 60000,
    },
    {
        ...
        answers: {
            ...
            olaj_szuro: "Minden szűrőt (levegő, pollen, üzemanyag)", parts_tier: "Márkás utángyártott (Bosch, TRW, Febi)",
        },
    ...
    },
    {
        ...
        answers: {
            ...
            km: "100–150 000 km", olaj_szuro: "Minden szűrőt (levegő, pollen, üzemanyag)", fek_hol: "Mind a kettő",
